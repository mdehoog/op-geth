...
type Transaction struct {
	...
	// cache of details to compute the L1 data fee
	l1CostData atomic.Value
}
...
// L1CostData caches the information needed to efficiently compute the L1 data fee even if the L1
// cost function changes.
func (tx *Transaction) L1CostData() L1CostData {
	if tx.Type() == DepositTxType {
		return L1CostData{}
	}
	if v := tx.l1CostData.Load(); v != nil {
		return v.(L1CostData)
	}
	...
	out := NewL1CostData(data)
	tx.l1CostData.Store(out)
	return out
}
