			...
			if list := pool.pending[addr]; list != nil {
				if tx := list.txs.Get(nonce); tx != nil {
					...
					if pool.l1CostFn != nil {
						if l1Cost := pool.l1CostFn(tx.L1CostData()); l1Cost != nil { // add rollup cost
...
func (pool *LegacyPool) reset(oldHead, newHead *types.Header) {
	...
	pool.l1CostFn = types.NewL1CostFunc(pool.chainconfig, statedb, newHead.Time)
...
func (pool *LegacyPool) promoteExecutables(accounts []common.Address) []*types.Transaction {
	...
	for _, addr := range accounts {
		...
		if !list.Empty() && pool.l1CostFn != nil {
			...
			if l1Cost := pool.l1CostFn(el.L1CostData()); l1Cost != nil {
