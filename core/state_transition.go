...
type Message struct {
	...
	L1CostData  types.L1CostData // L1CostData caches data to compute the fee we charge for L1 data availability
}
...
func (st *StateTransition) buyGas() error {
	...
	if st.evm.Context.L1CostFunc != nil && !st.msg.SkipAccountChecks {
		l1Cost = st.evm.Context.L1CostFunc(st.msg.L1CostData)
		if l1Cost != nil {
...
func (st *StateTransition) innerTransitionDb() (*ExecutionResult, error) {
	...
	if optimismConfig := st.evm.ChainConfig().Optimism; optimismConfig != nil && rules.IsOptimismBedrock {
		...
		if cost := st.evm.Context.L1CostFunc(st.msg.L1CostData); cost != nil {
