	...
	for i := 0; i < 3; i++ {
		...
		if bc != nil {
			...
			if err == nil {
				...
				context := core.NewEVMBlockContext(header, bc, nil, types.NewL1CostFunc(config, statedb))
			...
			context := core.NewEVMBlockContext(header, lc, nil, types.NewL1CostFunc(config, state))
			txContext := core.NewEVMTxContext(msg)
