		...
		go func() {
			...
			for task := range taskCh {
				var (
					...
					blockCtx = core.NewEVMBlockContext(task.block.Header(), api.chainContext(ctx), nil, types.NewL1CostFunc(api.backend.ChainConfig(), task.statedb))
				)
...
func (api *API) IntermediateRoots(ctx context.Context, hash common.Hash, config *TraceConfig) ([]common.Hash, error) {
	...
	var (
		...
		vmctx              = core.NewEVMBlockContext(block.Header(), api.chainContext(ctx), nil, types.NewL1CostFunc(chainConfig, statedb))
...
func (api *API) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	var (
		...
		blockCtx  = core.NewEVMBlockContext(block.Header(), api.chainContext(ctx), nil, types.NewL1CostFunc(api.backend.ChainConfig(), statedb))
		signer    = types.MakeSigner(api.backend.ChainConfig(), block.Number())
...
func (api *API) traceBlockParallel(ctx context.Context, block *types.Block, statedb *state.StateDB, config *TraceConfig) ([]*txTraceResult, error) {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				blockCtx := core.NewEVMBlockContext(block.Header(), api.chainContext(ctx), nil, types.NewL1CostFunc(api.backend.ChainConfig(), task.statedb))
...
func (api *API) TraceCall(ctx context.Context, args ethapi.TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, config *TraceCallConfig) (interface{}, error) {
	...
	vmctx := core.NewEVMBlockContext(block.Header(), api.chainContext(ctx), nil, types.NewL1CostFunc(api.backend.ChainConfig(), statedb))
